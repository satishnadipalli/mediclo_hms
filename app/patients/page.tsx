...
const PatientsPage: React.FC = () => {
  ...
  const normalizePatientData = (apiPatient: ApiPatient): Patient => {
    ...
    const gender =
      apiPatient.gender || apiPatient.childGender || "not_specified";
    ...
    if (apiPatient.latestAppointment) {
      const appointmentDate = new Date(
        apiPatient.latestAppointment.appointmentDate
      );
    ...
    if (!age && (apiPatient.dateOfBirth || apiPatient.childDOB)) {
      const birthDate = new Date(
        apiPatient.dateOfBirth || apiPatient.childDOB!
      );
  ...
  const updatePaymentStatus = async () => {
    ...
    try {
      ...
      if (updateResponse.data && updateResponse.data.patient) {
        const updatedPatientData = normalizePatientData(
          updateResponse.data.patient
        );
      ...
      alert(
        `Payment status updated successfully to ${
    ...
    <div className="p-6 max-w-[84%] font-sans mt-15 ml-70 mx-auto overflow-y-auto hide-scrollbar">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 flex-1">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={fetchPatients}
            className="text-sm text-[#C83C92] hover:text-[#B8358A] transition-colors"
          >
            Refresh
          </button>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredPatients.map((patient) => (
                ...
                >
                  <td className="py-4 px-2 text-[#456696] font-medium">
                    {patient.name}
                  </td>
                  ...
                  <td className="py-4 px-2">
                    {patient?.upcomingAppointment ? (
                      patient?.rawData?.latestAppointment?.paymentStatus ===
                      "pending" ? (
                        ...
                        <span className="bg-green-600 p-1 text-sm rounded-2xl text-white px-3">
                          Completed
                        </span>
                      )
                    ) : (
      ...
      {isModalOpen && selectedPatient && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-[#1E437A] mb-3">
                Payment Status
              </label>
